  ...
  return (
    <div className="stack" style={{ gap: 12 }}>
      <PracticeTitle
        ...
        goal="Goal: see whitespace pitfalls and choose correct assertion (toHaveText vs toContainText)."
      />

      <div className="card" style={{ padding: 12, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        <div className="stack" style={{ gap: 10 }}>
          <div className="card" style={{ padding: 12 }}>
            <div className="small">Counter</div>
            <div data-testid="counter" style={{ fontWeight: 900, fontSize: 22 }}>{count}</div>
            <div className="stack" style={{ gap: 8, marginTop: 10 }}>
              <button data-testid="btn-inc" className="btnPrimary" onClick={() => setCount((c) => c + 1)}>+1</button>
              <button data-testid="btn-reset" className="btnSecondary" onClick={() => setCount(0)}>Reset</button>
            </div>
          </div>

          <div className="card" style={{ padding: 12 }}>
            <div className="small">Text with extra spaces</div>
            <div data-testid="weird-text" style={{ fontWeight: 900 }}>{weirdText}</div>
          </div>
        </div>
      </div>

        <CodeBox
          code={`await page.getByTestId('btn-inc').click();
await expect(page.getByTestId('counter')).toHaveText('1');

await expect(page.getByTestId('weird-text')).toContainText('Hello');
// avoid exact match when whitespace is inconsistent`}
        />
    </div>
  );
}
